...
interface AtlasModelRecord {
  model?: unknown;
  type?: unknown;
  categories?: unknown;
  display_console?: unknown;
}
...
async function readJson(response: Response): Promise<AtlasEnvelope> {
  ...
  return payload as AtlasEnvelope;
}

function errorMessage(payload: AtlasEnvelope, fallback: string): string {
  return typeof payload.message === "string" && payload.message.trim()
    ? payload.message.trim()
    : fallback;
}

async function predictionData(response: Response): Promise<AtlasPredictionData> {
  ...
  return payload.data as AtlasPredictionData;
}

function predictionId(data: AtlasPredictionData): string {
...
function completedOutput(data: AtlasPredictionData): string | undefined {
  ...
  if (status === "completed") {
    if (
      ...
      throw new AtlasRequestError(
        "Completed Atlas Cloud prediction did not contain an output URL."
      );
    }
  ...
  if (typeof status !== "string" || !PENDING_STATUSES.has(status)) {
    throw new AtlasRequestError(
      `Atlas Cloud returned an unknown prediction status: ${String(status)}`
    );
  }
...
function mimeTypeForResponse(response: Response): string {
...
export async function fetchAtlasImageModels(
  _apiKey: string,
  ...
  try {
    ...
    return (payload.data as AtlasModelRecord[])
      .filter(
        (item) =>
          item.type === "Image" &&
...
export class AtlasImageClient implements ImageProviderClient {
  ...
  async generateImage(input: GenerateImageInput): Promise<GenerateImageResult> {
    ...
    try {
      ...
      const response = await this.fetchImpl(
        `${this.apiBase}/api/v1/model/generateImage`,
        {
          method: "POST",
          headers: {
      ...
      if (!imageResponse.ok) {
        throw new AtlasRequestError(
          `Atlas Cloud image download failed with HTTP ${imageResponse.status}.`,
  ...
  private async fetchPrediction(
    ...
    signal: AbortSignal
  ): Promise<AtlasPredictionData> {
    for (let attempt = 0; attempt <= MAX_PREDICTION_RETRIES; attempt += 1) {
      const response = await this.fetchImpl(
        `${this.apiBase}/api/v1/model/prediction/${encodeURIComponent(predictionIdValue)}`,
        {
          headers: {
            Authorization: `Bearer ${this.config.apiKey}`,
            "User-Agent": USER_AGENT,
